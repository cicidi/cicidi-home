...
public interface GeoDataRepository extends JpaRepository<GeoData, Long> {
    ...
    List<GeoData> findByCompanyAddress(@Param("companyName") String companyName, @Param("city") String city, @Param("state") String state, @Param("country") String country);

    @Query(value = "SELECT g from GeoData g inner join g.address a where g.companyName=:companyName and a.city=:city ")
    List<GeoData> findByCompanyAndCity(@Param("companyName") String companyName, @Param("city") String city);
